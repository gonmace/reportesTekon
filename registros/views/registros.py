...
class RegistrosViewSet(viewsets.ModelViewSet):
    """
    ViewSet completo para manejar todas las operaciones de Registros
    """
    ...
    def activos(self, request):
        ...
        """
        registros = self.queryset.filter(is_deleted=False)
        serializer = self.get_serializer(registros, many=True)
        return Response(serializer.data)
    ...
    def inactivos(self, request):
        ...
        """
        registros = self.queryset.filter(is_deleted=True)
        serializer = self.get_serializer(registros, many=True)
        return Response(serializer.data)
