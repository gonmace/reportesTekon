...
DataTable.ext.classes.container =
  "dt-container table table-sm !table-zebra w-full";
      ...
      ajax: {
        ...
        dataSrc: function (json) {
          ...
          if (json.detail && json.detail.includes("Authentication")) {
            ...
            // Verificar si hay una URL padre configurada
            const parentSystemUrl = window.PARENT_SYSTEM_URL || "";
            if (parentSystemUrl) {
              // Redirigir al login de la URL padre
              window.location.href = `${parentSystemUrl}/accounts/login/?next=${encodeURIComponent(
          ...
          if (json && json.results && Array.isArray(json.results)) {
            console.log(
              `API devolvió ${json.results.length} sitios de ${json.count} total`
            );
            return json.results;
          }
      ...
      fetch(`/api/v1/sitios/${siteId}/edit-modal/`, {
        ...
        .then((data) => {
          if (data.success) {
            ...
            const latInput = modalContent.querySelector(
              'input[name="lat_base"]'
            );
            const lonInput = modalContent.querySelector(
              'input[name="lon_base"]'
            );
              ...
              fetch(`/api/v1/sitios/${siteId}/edit-modal/`, {
                ...
                .then((data) => {
                  if (data.success) {
                    ...
                    Alert.error(
                      data.message || "Error al actualizar el sitio."
                    );
                  }
                })
                .catch((error) => {
