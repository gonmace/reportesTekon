...
class RAccesoView(LoginRequiredMixin, BreadcrumbsMixin, FormView):
    template_name = 'pages/r_acceso.html'
    form_class = RAccesoForm

    class Meta:
        title = 'Crear Registro Acceso'
        header_title = 'Crear Registro Acceso'
    ...
    def get_form_kwargs(self):
        ...
        if registro_id:
            ...
            # Verificar si ya existe un RAcceso para este registro
            try:
                registro_txtss = get_object_or_404(RegistrosTxTss, id=registro_id)
                # Buscar si ya existe un RAcceso para este registro
                racceso_existente = RAcceso.objects.filter(registro=registro_txtss).first()
                if racceso_existente:
                    # Si existe, usar esa instancia para pre-llenar el formulario
                    kwargs['instance'] = racceso_existente
